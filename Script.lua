...
-- [[ 🪐 MY CUSTOM SCRIPT HUB (15 SCRIPTS LOADED - MOBILE EDITION) 🪐 ]]
...
TitleBar.Text = "MY CUSTOM SCRIPT HUB 🚀 (15 Scripts)"  
...
local function CreateLauncherButton(parentTab, buttonText, targetGuiName, onClickFunc)
    ...
    Btn.MouseButton1Click:Connect(function()
        if not isLoaded then
            ...
            Btn.Text = " " .. buttonText .. (targetGuiName and " (ON)" or " (✅ Active)")
            task.spawn(onClickFunc)
        else
...
-- ====================================================================
-- [[ 📦 SCRIPT WRAPPERS (กล่องเก็บโค้ดดิบ 1-15) ]]
-- ====================================================================

-- 1. Infinite Camera Zoom
local function Script_InfiniteZoom()
    local Players = game:GetService("Players")
    local LocalPlayer = Players.LocalPlayer
...
local function Script_AirWalk()
    ...
    local Platform = nil
    local LockedY = 0 
    local ScreenGui = game:GetService("CoreGui"):FindFirstChild("ScreenGui_AirWalk") or Instance.new("ScreenGui", game:GetService("CoreGui"))
        ...
        for id, data in pairs(WarpStorage) do
            ...
            warpBtn.MouseButton1Click:Connect(function()
                if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                    player.Character.HumanoidRootPart.CFrame = data.CFrameValue + Vector3.new(0, 3, 0)
                end
            end)
...
local function Script_FreecamMobile()
    ...
    local dragging = false; local dragStart, startPos; local isMoved = false
    Button.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging = true; dragStart = input.Position; startPos = Button.Position; isMoved = false
    ...
    UserInputService.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            local delta = input.Position - dragStart
            if math.abs(delta.X) > 5 or math.abs(delta.Y) > 5 then isMoved = true end
...
local function Script_HybridNoclip()
    ...
    for _, gui in pairs(CoreGui:GetChildren()) do if gui.Name == "NoclipHubGui" then gui:Destroy() end end
...
local function Script_JoystickCustomizer()
    ...
    local dragging, dragStart, startPos
    SettingsHub.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging = true; dragStart = input.Position; startPos = SettingsHub.Position
        end
    end)
    UserInputService.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            local delta = input.Position - dragStart
            SettingsHub.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
        end
    end)
...
local function Script_FastRespawn()
    ...
    Button.MouseButton1Click:Connect(function()
        if isMoved or isCooldown then return end
        isCooldown = true
        local char = LocalPlayer.Character
        if char then
...
local function Script_ClickTeleport()
    ...
    local dragging = false; local dragStart, startPos; local hasMoved = false
    ToggleBtn.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging = true; dragStart = input.Position; startPos = ToggleFrame.Position; hasMoved = false
        end
    end)
    UserInputService.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
...
local function Script_SpeedModifier()
    local ScreenGui = game:GetService("CoreGui"):FindFirstChild("SpeedModifierGui") or Instance.new("ScreenGui", game:GetService("CoreGui"))
...
local function Script_MobileSpectate()
    ...
    local UserInputService = game:GetService("UserInputService"); local CoreGui = game:GetService("CoreGui")
...
local function Script_MegaCage()
    ...
    local ScreenGui = CoreGui:FindFirstChild("MegaCageGui") or Instance.new("ScreenGui", CoreGui)
    ...
    local dragging, dragStart, startPos
    CageButton.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging = true; dragStart = input.Position; startPos = CageButton.Position
        end
    end)
    UIS.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            local delta = input.Position - dragStart; CageButton.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
        end
    end)
...
local function Script_ServerHop()
    ...
    local ScreenGui = CoreGui:FindFirstChild("ServerHopGui") or Instance.new("ScreenGui", CoreGui)
...
-- ====================================================================
-- [[ 🛠️ MENU BUILDER (ประกอบปุ่มลงในแท็บ) ]]
-- ====================================================================

-- 1. สร้างหน้าแท็บต่างๆ
...
-- 2. ผูกฟังก์ชันเข้ากับปุ่มบน Hub

-- แท็บ PLAYER
CreateLauncherButton(PagePlayer, "ปลดล็อคซูมกล้อง (Infinite Zoom)", nil, Script_InfiniteZoom)
CreateLauncherButton(PagePlayer, "ปุ่มรีเซ็ตตัวละคร (Fast Respawn)", "FastRespawnMobileGui", Script_FastRespawn)

-- แท็บ MOVEMENT
CreateLauncherButton(PageMovement, "เดินบนอากาศ V4 (Air Walk)", "ScreenGui_AirWalk", Script_AirWalk)
CreateLauncherButton(PageMovement, "ทะลุกำแพง (Noclip Hub)", "NoclipHubGui", Script_HybridNoclip)
CreateLauncherButton(PageMovement, "สคริปต์บินแบบ Auto-Respawn", nil, Script_AutoFly)
CreateLauncherButton(PageMovement, "ปรับแต่งความเร็ว (Speed Modifier)", "SpeedModifierGui", Script_SpeedModifier)

-- แท็บ VISION
CreateLauncherButton(PageVision, "โหมดสอดแนม (Freecam Mobile)", "FreecamMobileGui", Script_FreecamMobile)
CreateLauncherButton(PageVision, "แอบดูผู้เล่นอื่น (Mobile Spectate)", "MobileSpectateV2GUI", Script_MobileSpectate)
CreateLauncherButton(PageVision, "มองทะลุกำแพง (Universal ESP)", nil, Script_UniversalESP)

-- แท็บ TELEPORT
CreateLauncherButton(PageTeleport, "เมนูเซฟจุดวาร์ป (Multi-Warp)", "MultiWarpGUI_V1", Script_MultiWarp)
CreateLauncherButton(PageTeleport, "แตะเพื่อวาร์ป (Click-to-Teleport)", "MobileClickTeleport", Script_ClickTeleport)

-- แท็บ CONTROLS
CreateLauncherButton(PageControls, "ปรับแต่งจอยสติ๊ก (Joystick Mod)", "JoystickCustomizerUI", Script_JoystickCustomizer)

-- แท็บ UTILITIES
CreateLauncherButton(PageUtils, "แฮ็กปุ่มกด (Instant Prompt + X-Ray)", nil, Script_ProximityHack)
CreateLauncherButton(PageUtils, "เสกกรงป้องกัน (Mega Cage)", "MegaCageGui", Script_MegaCage)

-- แท็บ SERVER
CreateLauncherButton(PageServer, "ย้ายเซิร์ฟเวอร์ / Rejoin", "ServerHopGui", Script_ServerHop)

print("✅ [Hub] Successfully Loaded 15 Scripts (Mobile Edition)!")
